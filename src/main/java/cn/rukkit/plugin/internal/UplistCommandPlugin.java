...
public class UplistCommandPlugin extends InternalRukkitPlugin implements EventListener {
    ...
    private static void getServerList() {
        for (String url : MASTER_SERVER_URLS) {
            String apiUrl = url + "/interface?action=list";
            System.out.println("Fetching server list from: " + apiUrl);
            ...
            try {
                ...
                // Parse server list section
                String[] lines = response.split("\n");
                boolean inServerList = false;
                List<String> serverList = new ArrayList<>();
                
                for (String line : lines) {
                    if (line.contains("[SERVERLIST]")) {
                        inServerList = true;
                        continue;
                    }
                    if (line.contains("[ENDSERVERLIST]")) {
                        inServerList = false;
                        break;
                    }
                    if (inServerList && !line.trim().isEmpty()) {
                        serverList.add(line.trim());
                    }
                }
                
                if (!serverList.isEmpty()) {
                    System.out.println("Found server list:");
                    serverList.forEach(System.out::println);
                    return;
                }
            } catch (IOException e) {
                System.out.println("Error fetching server list from " + url + ": " + e.getMessage());
            }
        }
        
        System.out.println("No server lists found from any master server");
    }
    ...
    try {
        ...
        if (PROXY_ENABLED) {
            Proxy proxy = new Proxy(Proxy.Type.HTTP, 
                                  new InetSocketAddress("micro.xin", 3128));
...
private static String getResponseHeaders(HttpURLConnection conn) {
    ...
    public void onLoad() {
        ...
        mgr.registerServerCommand(new ServerCommand("uplist", LangUtil.getString("chat.debug"), 1, new UplistCallback(), this));
        mgr.registerServerCommand(new ServerCommand("uplistSvc", LangUtil.getString("chat.debug"), 1, new UplistAndEnableUpdateServiceCallback(), this));
        mgr.registerServerCommand(new ServerCommand("downlist", LangUtil.getString("chat.debug"), 1, new DownlistCallback(), this));
        mgr.registerServerCommand(new ServerCommand("EnableUpdateService", "Enable periodic server list updates", 1, new EnableUpdateServiceCallback(), this));
        mgr.registerServerCommand(new ServerCommand("DisableUpdateService", "Disable periodic server list updates", 1, new DisableUpdateServiceCallback(), this));
